...
export default function GratidudeMessage({ gratitude }: { gratitude: Gratitude }) {
  ...
  const editGratitudeMessage = useCallback<any>(() => {
    console.log("todo")
  }, [])
  ...
  const deleteGratitudeMessage = useCallback<any>(() => {
    console.log("todo")
  }, [])
    ...
    <div className='gm-root'>
      ...
      <div className="rest-column">
        <div style={{
          ...
          <div style={{ display: "flex" }}>
            <Tooltip label="Edit">
              <ActionIcon variant="outline" radius={16} onClick={() => editGratitudeMessage()}>
      ...
      {/* TODO: inline styles are better to navigate */}
      <style jsx>
        {`
          .gm-root {
            max-width: 600px;
